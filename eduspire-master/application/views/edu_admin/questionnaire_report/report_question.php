                                    ...
                                    foreach($answer_array[$j] as $response_answers)
                                    {
                                        ...
                                        foreach($value as $ans)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                if(isset($value[$m]['answer']))
                                                {
                                                    //$k is the answer number from question_result table
                                                    $question_ans_records[$response_answers][] =  '<a href="'.base_url().'edu_admin/questionnaire_report/view_responses/'.$ans['qrAssignID'].'/'.$k.'/'.$ans['qrID'].'" class="fancybox" id="comments_box">View Responses</a>';break;
                                                }
                                            }
                                        }
                                        $m++;
                                   ...
                                   //value=the answers count given by students from questionnaire_result table
                                    ...
                                    foreach($question_ans_records as $ans_key=>$ans_value)
                                    {
                                       ...
                                       else
                                       {
                                                ...
                                                $key_sec .= '<td><a href="'.base_url().'edu_admin/questionnaire_report/view_users/'.$ans['qrAssignID'].'/'.$ans['qr_field'].'/'.$ans_opt_position.'" class="fancybox" id="comments_box">'.traverse_array($ans_value).'</a></td>';
